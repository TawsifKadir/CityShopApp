...
class CountryDropdown extends StatelessWidget {
  ...
  const CountryDropdown({required this.country, required this.countryList, required this.onChanged});
  ...
  String get _dropdownHint {
    if (country == null || !countryList.haveList) return 'Select your country'.translate;
    var index = countryList.indexWhere((item) => item.id == country!.id);
    return index < 0 ? 'Select your country'.translate : countryList[index].country_name;
  }

  List<DropdownMenuItem<Country>> get _dropdownMenuItems {
    List<DropdownMenuItem<Country>> menuItems = [];
    if (!countryList.haveList) return menuItems;
    countryList.forEach((item) => menuItems.add(_menuItem(item)));
    return menuItems;
  }
  ...
  DropdownMenuItem<Country> _menuItem(Country item) {
    ...
    var label = Text('$empty_space${item.country_name}', maxLines: 1, overflow: overflow, style: style);
