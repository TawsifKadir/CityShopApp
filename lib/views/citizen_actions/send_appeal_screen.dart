...
class SendAppealScreen extends StatefulWidget {
  final Complain complain;
  const SendAppealScreen({required this.complain});

  @override
  State<SendAppealScreen> createState() => _SendAppealScreenState();
}

class _SendAppealScreenState extends State<SendAppealScreen> {
  var viewModel = SendAppealViewModel();
  var modelData = SendAppealViewModel();
  ...
  void didChangeDependencies() {
    viewModel = Provider.of<SendAppealViewModel>(context, listen: false);
    modelData = Provider.of<SendAppealViewModel>(context);
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        title: Text('See For Appeal'.translate),
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          ...
          Text('Are you sure you want to appeal?'.translate, style: sl<TextStyles>().text18_600(black)),
          const SizedBox(height: 10),
          Text('Your Note'.translate, style: sl<TextStyles>().text16_400(black)),
          const SizedBox(height: 6),
          ModifiedTextField(minLines: 5, maxLines: 12, controller: viewModel.note, hintText: hint),
