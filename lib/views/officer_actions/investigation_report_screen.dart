...
class InvestigationReportScreen extends StatefulWidget {
  ...
  const InvestigationReportScreen({required this.complain, required this.viewPref, required this.typePref});

  @override
  State<InvestigationReportScreen> createState() => _SubmitInvestigationScreenState();
}

class _SubmitInvestigationScreenState extends State<InvestigationReportScreen> {
  var viewModel = SubmitInvestigationViewModel();
  var modelData = SubmitInvestigationViewModel();
  ...
  void didChangeDependencies() {
    viewModel = Provider.of<SubmitInvestigationViewModel>(context, listen: false);
    modelData = Provider.of<SubmitInvestigationViewModel>(context);
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        title: Text('Submit Investigation Report'.translate),
  ...
  Widget _mobileView(BuildContext context) {
    var hint = 'Write your report here'.translate;
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          ...
          Text('Report for investigation'.translate, style: sl<TextStyles>().text16_400(black)),
          const SizedBox(height: 6),
          ModifiedTextField(minLines: 06, maxLines: 12, controller: viewModel.report, hintText: hint),
          const SizedBox(height: 20),
          AttachmentView(attachmentList: attachment ?? []),
          const SizedBox(height: 20),
          Text('attention_for_submit_investigation'.translate, style: sl<TextStyles>().text13_400(red)),
          const SizedBox(height: 6),
          ProofFilesView(),
          const SizedBox(height: 20),
        ],
